...
exports.getCartProducts = async (req, res) => {
  if (req.session.user) {
    ...
    try {
      ...
      let total = await Cart.aggregate([
        ...
        },
        {
          $project: {
            ...
            tax: 1,
            productInfo: { $arrayElemAt: ["$productInfo", 0] },
            totalTax: { $multiply: ['$quantity', '$tax'] },
            total: { $multiply: ['$quantity', '$currentPrice'] },
            totalWithTax: { $multiply: ['$quantity', { $add: ['$tax', '$currentPrice'] }] }
          },
        },
        {
          $group: {
      ...
      console.log(total, "total sillumon");
      ...
      if (cartItems.length === 0) {
        ...
        console.log('Calculated totals:', total[0].total);
...
exports.changeProductQuantity = async (req, res) => {
  ...
  try {
    ...
    let quanttity = req.body.quantity
    let unique_id = new ObjectId(req.body.product)
    count = parseInt(count)
    quanttity = parseInt(quanttity)


    if (count == -1 && quanttity == 1) {
      ...
      console.log(total, "calculated total sillu");
...
exports.orderPlaced = async (req, res) => {
  ...
  await res.render('user/orderPlaceCod', {
    userloggedIn: req.session.userloggedIn,
    user,
    username,
    cartCount,
